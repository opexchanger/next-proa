...
import travels from '../../../data/travels.preval';

import styles from './buy-form.module.scss';

export default function BuyForm({ data }) {
  const {
    subjects,
    btnEmailText,
    btnWhatsappText
      ...
      {({ isValid, dirty, isSubmitting, status }) => (
        ...
        !status ?
          <Form className={`${styles.form} ${isSubmitting ? styles.submitting : ''}`}>
            ...
            <Label htmlFor="travels">Já escolheu o destino que está interessado?</Label>
            <SelectField id="travels" name="travels">
              <option key="1" value="Nenhum">Ainda não</option>
              {travels.map(({ slug, title }) => (
